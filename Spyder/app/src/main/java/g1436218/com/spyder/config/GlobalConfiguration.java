package g1436218.com.spyder.config;

public class GlobalConfiguration {

    public static final int BLUETOOTH_TIME_INTERVAL = 15;
    //Doc VM
    public static final String IP = "146.169.46.38";
    //Supervisor VM
    //public static final String IP = "146.169.32.147";
    public static final int PORT = 55555;
    public static final String DEFAULT_URL = "http://" + IP + ":" + PORT + "/MongoDBWebapp/";
    public static final int NUMBER_OF_MINI_BATCHES = 4;
    public static final String PROJECT_NUMBER = "139712018020";
    public static final int VIBRATE_DURATION = 1000;



    public static int nid = 0;

}
